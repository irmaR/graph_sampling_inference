...
def main_multiple_pattern_size_plots(pattern_sizes,path_to_csv,exp,step_percentage,m,n,eligibility=True):
    ...
    for i in range(len(pattern_sizes)-1):
        ...
        for pattern in map_runtimes_seconds.keys():
            if pattern in eligble_patterns:
                ...
                if not exp=='dblp':
                  map_furer_AD_10_to_percentages_rel_errors[pattern]=get_results_percentages(pattern,map_runtimes_seconds[pattern],map_final_embs[pattern],os.path.join(path_to_csv,'Ffurer_order_random_'+str(level)+".csv"),exp,step_percentage)
        ...
        avg_ERR_AD_10=get_average_errors_per_percentage(map_furer_AD_10_to_percentages_rel_errors,step_percentage)   
        avg_ERR_random=get_average_errors_per_percentage(map_random_to_percentages_rel_errors,step_percentage)   
        ...
        stdev_furer_AD_10=get_stdev_per_percentage(map_furer_AD_10_to_percentages_rel_errors,step_percentage)
        ...
        print "Stdev obd: level",level,stdev_furer_OBD
        print "Stdev ad:",level,stdev_furer_AD
        print "Stdev ad-10:",level,stdev_furer_AD_10
        print "Stdev random:",level,stdev_random

                 
        print "Avg Furer: ",avg_ERR_AD_10
        ...
        if i == 4:
           ...
           ax.set_ylabel('Avg RelErr',size=25)
        else:
          ax = fig.add_subplot(iplot)
        ...
        ax.plot(x,avg_ERR_OBD,'-o',color='blue',linewidth=2.0,markersize=10)
        ax.plot(x,avg_ERR_AD,':v',color='red',linewidth=2.0,markersize=10)
        ax.plot(x,avg_ERR_AD_10,':s',color='black',linewidth=2.0,markersize=10)
        ax.plot(x,avg_ERR_random,'--D',color='green',linewidth=2.0,markersize=10)
        ...
        ax.errorbar(x, avg_ERR_AD,yerr=stdev_furer_AD,color='red',linewidth=2.0,ls=':')
        if not exp=='dblp':
          ax.errorbar(x, avg_ERR_AD_10,yerr=stdev_furer_AD,color='black',linewidth=2.0,ls=':')
        ax.errorbar(x, avg_ERR_random,yerr=stdev_random,color='green',linewidth=2.0,ls='--')
        ax.set_ylim([0,1])
        ax.set_xlim(xmin=0)
        ...
        if i==0:
            if not exp=='dblp':
               ax.legend(['FK-OBD', 'FK-AD','FK-AD-10','Random'],loc = 'upper right', bbox_to_anchor = (0,0.5,1.02,0.5),prop={'size':15}, ncol=2)
            else:
               ax.legend(['FK-OBD', 'FK-AD','Random'],loc = 'upper right', bbox_to_anchor = (0,0.5,1.02,0.5),prop={'size':15}, ncol=2)
    ...
    if exp=="dblp":
        plt.title("DBLP", fontsize=30,y=1.06,x=0.45)
    elif exp=="yeast":
        plt.title("YEAST", fontsize=30,y=1.06,x=0.45)
    elif exp=="webkb":
        plt.title("WEBKB", fontsize=30,y=1.06,x=0.45)
    elif exp=="facebook":
        plt.title("FACEBOOK", fontsize=30,y=1.06,x=0.45)
    elif exp=="imdb":
        plt.title("IMDB", fontsize=30,y=1.06,x=0.45)
    elif exp=="am":
        plt.title("AMAZON", fontsize=30,y=1.06,x=0.45)
    plt.show()
...
if __name__=='__main__':
  ...
  main_multiple_pattern_size_plots([4,5,6,7,8,9,10],'/home/irma/work/RESULTS/graph_sampling/facebook_csvs/','facebook',10,2,2,eligibility=False)
  #main_multiple_pattern_size_plots([4,5,6,7,8,9,10],'/home/irma/work/RESULTS/graph_sampling/webkb_csvs/','webkb',10,2,2,eligibility=False)
  #main_multiple_pattern_size_plots([4, 5, 6, 7, 8, 9, 10], '/home/irma/work/RESULTS/graph_sampling/yeast_csvs/','yeast', 10, 2, 2, eligibility=False)
  #main_multiple_pattern_size_plots([4, 5, 6, 7, 8, 9, 10], '/home/irma/work/RESULTS/graph_sampling/imdb_csvs/','imdb', 10, 2, 2, eligibility=False)
