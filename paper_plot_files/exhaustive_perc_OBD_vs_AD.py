...
def main_multiple_pattern_size_plots(pattern_sizes,path_to_csv,exp,step_percentage,m,n,eligibility=True):
    ...
    for i in range(4):
        ...
        ax.plot(x,avg_ERR_OBD,'-o',color='blue',linewidth=3.0,markersize=12)
        ax.plot(x,avg_ERR_AD,':v',color='red',linewidth=3.0,markersize=12)
        print 'plotting: ',exp
        print avg_ERR_OBD
        print avg_ERR_AD
        ax.errorbar(x, avg_ERR_OBD,yerr=stdev_furer_OBD,color='blue',linewidth=2)
        ax.errorbar(x, avg_ERR_AD,yerr=stdev_furer_AD,color='red',linewidth=2.0)
        ax.set_ylim([0,1])
...
if __name__=='__main__':
  ...
  main_multiple_pattern_size_plots([11, 12, 13, 14, 15], '/home/irma/work/RESULTS/graph_sampling/yeast_csvs/', 'yeast',10, 2, 2, eligibility=False)