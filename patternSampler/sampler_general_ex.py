        ...
        if P.node[Plist[i]]['target'] == True:
            ...
            for n in D.nodes():
                if D.node[n]['predicate'] == predicate1:
                    s.add(D.node[n]['value'])
#            print "here are all the possible values for predicate %s :\n" % predicate1
#            for el in s:
#                print el
            ##
...
def sampling_exhaustive_general_inf(D,  P,  Plist,  root_nodes):
    """A procedure that is GENERAL and can sample general graphs for patterns
    D : domain graph (networkx graph with 'predicate' and 'value' attributes)
    P : pattern graph (networkx graph with 'predicate' and 'value' attributes, and 'target' boolean value)
    Plist : ordered list of P nodes
...
def handle_quota(D,  P,  Plist,  total_freq_dict,  mappings_list):
    global globalist_randomnode
    global globaltimes_randomnode
    globaltimes_randomnode.append(time.time() - globaltimes_randomnode[0])
    ...
    globalist_randomnode.append(total_freq_dict)

    


def rec_fit_limited(n, D,  P,  Plist,  i,  matched_list,  nlimitlist,  nodes_observed_list,  total_freq_dict):
    matching_subgraphs = []
    if i < len(Plist):       #border condition
        if matches(n, D,  Plist[i], P,  matched_list,  Plist):
            matched_list.append(D.node[n]['id'])
            ...
            for c in candidates:
                ...
                if nodes_observed_list[0] in nlimitlist:    #intermediate or final quota: make copies and call procedure to handle them
                    ...
                    handle_quota(D,  P,  Plist, total_freq_dict_copy,  matching_subgraphs_copy)
                if nodes_observed_list[0] <= max(nlimitlist):
                    copyforcandidate = copy.deepcopy(matched_list)
                    result = rec_fit_limited(c,  D,  P,  Plist,  i+1,  copyforcandidate,  nlimitlist,  nodes_observed_list,  total_freq_dict)
                    if result != None:
                        for list in result:                        
                            if list not in matching_subgraphs:
                                matching_subgraphs.append(list)
...
def handle_quota_Furer(D,  P, total_Zlist_dict_copy, result_to_give, iteration_counter):
    global globalist_furer
    global globaltimes_furer
    globaltimes_furer.append(time.time() - globaltimes_furer[0])
    freq_dict = {}
    ...
    globalist_furer.append(freq_dict)




def rec_fit_Furer(nodes, D,  P,  OBdecomp,  i,  matched_list,  nlimitlist,  nodes_observed_list, Zlist_dict, iteration_counter,  epsilon_from_above):
    """
    ...
    if nodes_observed_list[0] > nlimitlist[cqi]:
        ...
        handle_quota_Furer(D,  P, total_Zlist_dict_copy, result_to_give, iteration_counter)
...
##                        if D.node[c]['predicate'] == P.node[el]['predicate']:                             #    the change to IZO-MATCH test the candidates
            ...
            if number_of_combinations > 0:
                ...
                copyforcandidate = copy.deepcopy(matched_list)
                result = rec_fit_Furer(nodes_next,  D,  P,  OBdecomp,  i+1,  copyforcandidate,  nlimitlist,  nodes_observed_list,  Zlist_dict, iteration_counter,  epsilon)
...
##        else:
##            # in this case give zeros to all...this is the correction of the "relaxed" version          --- WHAT IF some are not in Zlist yet? Optimized option without list, with counters would be better anyway
##            for key in Zlist_dict:
##                    Zlist_dict[key].append(0)
...
##    final_verdict = False
##    ##print "---pridemo v matches()"
##    if D.node[n]['predicate'] != P.node[m]['predicate']:
##        ##print "---niti predikat se ne ujema, konec."
##        return False
##    else:
##        test_list = matched_list + [n]
##        ##print "D-jeva vozlisca:",  test_list
##        sgD = D.subgraph(test_list)
##        ##print "P-jeva vozlisca:", Plist[:len(test_list)]
##        sgP = P.subgraph(Plist[:len(test_list)])    # we take as much from Plist as there is in test list
##        question = nx.is_isomorphic(sgD, sgP ,  node_match=predicate_match)
##        ##if question:
##            ##print "ocitno JE IZOMORFNO."
##        neighbor_test = False
##        njevi_sosedi = sgD.neighbors(n)
##        mjevi_sosedi = sgP.neighbors(m)
##        # indeksi seznama test_list in seznama Plist[:len(test_list)]  se morajo ujemati pri vseh sosedih od m-ja (ne od n-ja, ta jih ima lahko vec)
##        m_indeces = []
##        for ms in mjevi_sosedi:
##            m_indeces.append(Plist.index(ms))
##        n_indeces = []
##        for ns in njevi_sosedi:
##            n_indeces.append(test_list.index(ns))
##        ##print "m_indeces:",  m_indeces
##        ##print "n_indeces:",  n_indeces
##        if set(m_indeces) == set(n_indeces):
##            neighbor_test = True
##            ##print "neighbor_test je TRUE"
##        ##else:
##            ##print "neighbor_test je FALSE"
##        if neighbor_test and question:
##            final_verdict = True
...
def matches_Furer(n, D,  m,  P,  matched_list,  OBmatched):
    if predicate_match(D.node[n],  P.node[m]):
        test_list = matched_list + [n]
        #sgD = D.subgraph(test_list)
        #sgP = P.subgraph(OBmatched+[m])    # we add the one compared from decomposition position corresponding to n
        compare_list = OBmatched+[m]
        if len(test_list) !=len(OBmatched+[m]): #hence, they should be of equal size and ready to be compared
            print "WARNING: you are comparing graphs of non-equal size in matches_Furer() !"
        neighbor_test = True
        for m_neigh in P.neighbors(m):
