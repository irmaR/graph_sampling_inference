...
def collect_KLDs_for_pattern_size_average(path_to_exhaustive_csv,pattern_size,path_to_approach):
                  ...
                  for row1 in reader1:
                      ...
                      if(name_of_patterns1==name_of_patterns2):
                          for i in range(1,121):
                              value=row1["KLD_"+str(i)]
                              if not value:
                                   array_of_final_kld_approachX.append(float('nan'))
                              elif value==None:
                                  array_of_final_kld_approachX.append(float('nan'))
                              elif value=="None":
                                  array_of_final_kld_approachX.append(float('nan'))
                              else:
                                  array_of_final_kld_approachX.append(float(value))
   return array_of_final_kld_approachX 
         ...
         with open(path_to_approach) as appX:
              ...
              for row1 in reader1:                 
                  ...
                  if(name_of_patterns2==name_of_patterns):
                      ...
                      array_of_final_nr_emb_approachX.append(np.nanmean(array_values_appx)) 
                      break
...
def collect_EMB_for_pattern_size_final(path_to_exhaustive_csv,pattern_size,path_to_approach):
   array_of_nr_emb_exhaustive=[]
   array_of_final_nr_emb_approachX=[]
   number_of_patterns=0
   row_number=0
         ...
         with open(path_to_approach) as appX:
              ...
              for row1 in reader1:                 
                  ...
                  if(name_of_patterns2==name_of_patterns):
                      ...
                      value=row1["emb_120"]
                      if value:
                         array_of_final_nr_emb_approachX.append(float(value)) 
                      else:
                         array_of_final_nr_emb_approachX.append(float('nan')) 
                      break
...
def collect_stat_relative_error_average_results(file_name,dataset_name,path_to_csvs,pattern_sizes,metric):
    ...
    FK_OBD_per_pattern_size={}
    FK_AD_per_pattern_size={}
    FK_AD_ord_per_pattern_size={}
    random_per_pattern_size={}
    
    FK_OBD_per_pattern_size_std={}
    FK_AD_per_pattern_size_std={}
    FK_AD_ord_per_pattern_size_std={}
    random_per_pattern_size_std={}
    ...
    for n in pattern_sizes:
        ...
        print "furer ..."
        if metric=="emb":
           rmses=collect_EMB_for_pattern_size_average(path_to_exhaustive_csv,patt_size,path_to_Furer)
           RMSEFurerEMB.extend(rmses)
           FK_OBD_per_pattern_size[n]=rmses
        elif metric=="kld":
           klds1=collect_KLDs_for_pattern_size_final(path_to_exhaustive_csv,patt_size,path_to_Furer)
           RMSEFurerEMB.extend(klds1)
           FK_OBD_per_pattern_size[n]=klds1
          
        print "ffurer ..."
        if metric=="emb":
           rmses=collect_EMB_for_pattern_size_average(path_to_exhaustive_csv,patt_size,path_to_FFurer)
           RMSEFFurerEMB.extend(rmses)
           FK_AD_per_pattern_size[n]=rmses
        elif metric=="kld":
           klds2=collect_KLDs_for_pattern_size_final(path_to_exhaustive_csv,patt_size,path_to_FFurer)
           RMSEFFurerEMB.extend(klds2)
           FK_AD_per_pattern_size[n]=klds2
           #FK_AD_per_pattern_size_std[n]=np.nanstd(klds2);
        
#         print "ffurer ordered..."
#         if metric=="emb":
#            rmses=collect_EMB_for_pattern_size_average(path_to_exhaustive_csv,patt_size,path_to_FFurer_ord)
#            RMSEFFurer_ordEMB.extend(rmses)
#         elif metric=="kld":
#            klds3=collect_KLDs_for_pattern_size_final(path_to_exhaustive_csv,patt_size,path_to_FFurer_ord)
#            RMSEFFurer_ordEMB.extend(klds3)
#            FK_AD_ord_per_pattern_size[n]=klds3
#            #FK_AD_ord_per_pattern_size_std[n]=np.nanstd(klds3)
         
        print "random ..."
        if metric=="emb":
            rmses=collect_EMB_for_pattern_size_average(path_to_exhaustive_csv,patt_size,path_to_Random)
            RMSERandomEMB.extend(rmses)
            random_per_pattern_size[n]=rmses
        elif metric=="kld":
            klds4=collect_KLDs_for_pattern_size_final(path_to_exhaustive_csv,patt_size,path_to_Random)
            RMSERandomEMB.extend(klds4)
            random_per_pattern_size[n]=klds4
            #random_per_pattern_size_std[n]=np.nanstd(klds4)
        y_axis=None
        if metric=="emb":
            y_axis="Avg_RelErr"
        elif metric=="kld":
    ...
    print "Furer: AVG Rel Error: ",round(np.nanmean(RMSEFurerEMB),3),round(np.nanstd(RMSEFurerEMB),3)
    print "FFurer: AVG Rel Error: ",round(np.nanmean(RMSEFFurerEMB),3),round(np.nanstd(RMSEFFurerEMB),3)
    print "FFurer order: AVG Rel Error: ",round(np.nanmean(RMSEFFurer_ordEMB),3),round(np.nanstd(RMSEFFurer_ordEMB),3)
    print "Random: AVG Rel Error: ",round(np.nanmean(RMSERandomEMB),3),round(np.nanstd(RMSERandomEMB),3)
    FK_OBD_row=""
    FK_AD_row=""
    random_row=""
    FK_OBD_row1=""
    ...
    for i in pattern_sizes:
        klds_FK_OBD=FK_OBD_per_pattern_size[i]
        #std_FK_OBD=FK_OBD_per_pattern_size_std[i]
        klds_FK_AD=FK_AD_per_pattern_size[i]
        #std_FK_AD=FK_AD_per_pattern_size_std[i]
        klds_FK_random=random_per_pattern_size[i]
        #std_FK_random=random_per_pattern_size_std[i]
        #FK_OBD_row+=str(round(klds_FK_OBD,3))+"&"+str(round(std_FK_OBD,3))+"&"
        #FK_AD_row+=str(round(klds_FK_AD,3))+"&"+str(round(std_FK_AD,3))+"&"
        #random_row+=str(round(klds_FK_random,3))+"&"+str(round(std_FK_random,3))+"&"
        ...
        FK_OBD_row1+=str(np.nanmean(klds_FK_OBD))+"&"
    ...
    print np.nanmean(FK_OBD_avg)
    print FK_OBD_row1
    print round(1.86282217123e-03,3)
#     print "Averages per pattern size"
#     print FK_OBD_per_pattern_size
#     print "----------------------"
#     print FK_AD_per_pattern_size
#     print "----------------------"
#     print random_per_pattern_size
#     
#     print "STDs per pattern size"
#     print FK_OBD_per_pattern_size_std
#     print "----------------------"
#     print FK_AD_per_pattern_size_std
#     print "----------------------"
#     print random_per_pattern_size_std
...
if __name__ == '__main__':
    ...
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","YEAST","/home/irma/workspace/DMKD_Paper_Sampling/yeast_csvs/",[4,5,6,7,8,9,10],"kld")
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","WEBKB","/home/irma/workspace/DMKD_Paper_Sampling/webkb_csvs/",[4,5,6,7,8,9,10],"kld")
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","IMDB","/home/irma/workspace/DMKD_Paper_Sampling/imdb_csvs/",[4,5,6,7,8,9,10],"kld")
    
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","DBLP","/home/irma/workspace/DMKD_Paper_Sampling/dblp_csvs/",[4,5,6,7,8,9,10],"kld")
    
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","WEBKB","/home/irma/workspace/DMKD_Paper_Sampling/webkb_csvs/",[4,5,6,7,8,9,10],"kld")

    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","YEAST","/home/irma/workspace/DMKD_Paper_Sampling/yeast_csvs/",[4,5,6,7,8,9,10],"emb")
    ...
    collect_stat_relative_error_average_results("fb_avg_KLD_All.png","DBLP","/home/irma/workspace/DMKD_Paper_Sampling/dblp_csvs/",[4,5,6,7,8,9,10],"kld")
        ...
        #EMB
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","YEAST","/home/irma/workspace/DMKD_Paper_Sampling/yeast_csvs/",[4,5,6,7,8,9,10],"emb")
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","WEBKB","/home/irma/workspace/DMKD_Paper_Sampling/webkb_csvs/",[4,5,6,7,8,9,10],"emb")
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","IMDB","/home/irma/workspace/DMKD_Paper_Sampling/imdb_csvs/",[4,5,6,7,8,9,10],"emb")
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","FACEBOOK","/home/irma/workspace/DMKD_Paper_Sampling/facebook_csvs/",[4,5,6,7,8,9,10],"emb")
    #collect_stat_relative_error_average_results("fb_avg_KLD_All.png","DBLP","/home/irma/workspace/DMKD_Paper_Sampling/dblp_csvs/",[4,5,6,7,8,9,10],"emb")
