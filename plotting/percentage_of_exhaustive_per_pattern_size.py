...
def main_multiple_pattern_size_plots(pattern_sizes,path_to_csv,exps,paths,step_percentage,m,n,eligibility=True):
    ...
    column_num=3
    iplot=0
    FK_OBD_experiment_rel_err={}
    FK_AD_experiment_rel_err={}
    random_experiment_rel_err={}
    
    FK_OBD_experiment_std={}
    FK_AD_experiment_std={}
    random_experiment_std={}
    ...
    for exp in exps:
        ...
        path_to_csv=paths[counter_exp]
        counter_exp+=1
        
        all_levels_std_relative_errors_furer_OBD={}
        all_std_relative_errors_furer_AD={}
        all_std_relative_errors_furer_AD_10={}
        all_std_relative_errors_random={}
        for level in pattern_sizes:
             ...
             for pattern in map_runtimes_seconds.keys():
                if pattern in eligble_patterns:
                    ...
                    tmp_OBD[pattern]=get_results_percentages(pattern,map_runtimes_seconds[pattern],map_final_embs[pattern],os.path.join(path_to_csv,'furer_'+str(level)+".csv"),exp,step_percentage)
                    tmp_AD[pattern]=get_results_percentages(pattern,map_runtimes_seconds[pattern],map_final_embs[pattern],os.path.join(path_to_csv,'Ffurer_'+str(level)+".csv"),exp,step_percentage)
                    tmp_random[pattern]=get_results_percentages(pattern,map_runtimes_seconds[pattern],map_final_embs[pattern],os.path.join(path_to_csv,'random_'+str(level)+".csv"),exp,step_percentage)
             
             all_levels_average_relative_errors_furer_OBD[level]=get_average_errors_per_percentage(tmp_OBD,step_percentage)
             all_average_relative_errors_furer_AD[level]=get_average_errors_per_percentage(tmp_AD,step_percentage)
             all_average_relative_errors_random[level]=get_average_errors_per_percentage(tmp_random,step_percentage)
             
             all_levels_std_relative_errors_furer_OBD[level]=get_stdev_per_percentage(all_levels_average_relative_errors_furer_OBD,step_percentage)
             all_std_relative_errors_furer_AD[level]=get_stdev_per_percentage(all_average_relative_errors_furer_AD,step_percentage)
             all_std_relative_errors_random[level]=get_stdev_per_percentage(all_average_relative_errors_random,step_percentage)
            
             FK_OBD_experiment_rel_err[exp]=all_levels_average_relative_errors_furer_OBD
             FK_AD_experiment_rel_err[exp]=all_average_relative_errors_furer_AD
             random_experiment_rel_err[exp]=all_average_relative_errors_random
             
             FK_OBD_experiment_std[exp]=all_levels_std_relative_errors_furer_OBD;
             FK_AD_experiment_std[exp]=all_std_relative_errors_furer_AD;
             random_experiment_std[exp]=all_std_relative_errors_random;

             
    print FK_OBD_experiment_rel_err
    plot_counter=0
    
    for exp in exps:
        for level in pattern_sizes:
            iplot+=1
            avg_ERR_OBD=FK_OBD_experiment_rel_err[exp][level]
            avg_ERR_AD=FK_AD_experiment_rel_err[exp][level]
            avg_ERR_random=random_experiment_rel_err[exp][level]
            
            stdev_furer_OBD=FK_OBD_experiment_std[exp][level]
            stdev_furer_AD=FK_AD_experiment_std[exp][level]
            stdev_furer_random=random_experiment_std[exp][level]
            ...
            print iplot
            ax = fig.add_subplot(row_num,column_num,iplot)
            if iplot==1:
                ax.set_ylabel('YEAST \n Avg RelErr',size=20)
            if iplot==4:
                ax.set_ylabel('IMDB \n Avg RelErr',size=20)
            ...
            if iplot==1:
              ax.set_title('pattern size: 4',size=20)
            
            if iplot==2:
              ax.set_title('pattern size: 7',size=20)
            
            if iplot==3:
              ax.set_title('pattern size: 10',size=20)
            
            #x=xrange(5,605,15)
            ax.plot(x,avg_ERR_OBD,'-o',color='blue',linewidth=2.0)
            ax.plot(x,avg_ERR_AD,':v',color='red',linewidth=2.0)
            ax.plot(x,avg_ERR_random,'--D',color='green',linewidth=2.0)
            print 'plotting: ',exp
            print avg_ERR_OBD
            print avg_ERR_AD
            print avg_ERR_random
            ax.errorbar(x, avg_ERR_OBD,yerr=stdev_furer_OBD,color='blue',linewidth=2,ls='-')
            ax.errorbar(x, avg_ERR_AD,yerr=stdev_furer_AD,color='red',linewidth=2,ls=':')
            ax.errorbar(x, avg_ERR_random,yerr=stdev_furer_random,color='green',linewidth=2,ls='--')
            ax.set_ylim(ymin=0)
            max_yticks = 4
            yloc = plt.MaxNLocator(max_yticks)
            ax.yaxis.set_major_locator(yloc)
            plt.locator_params(axis='y', nticks=4)
            zed = [tick.label.set_fontsize(15) for tick in ax.yaxis.get_major_ticks()]
            zed = [tick.label.set_fontsize(15) for tick in ax.xaxis.get_major_ticks()]
            #ax.set_xticklabels(x,fontsize=25)
            #ax.set_xticklabels(x,fontsize=20)
            if iplot==15:
                plt.legend(['FK-OBD', 'FK-AD','Random'],loc = 'lower right', bbox_to_anchor = (0.45,-0.9,1.0,0.01),prop={'size':15},ncol=2)  
...
def main_multiple_pattern_size_plots_one_plot(pattern_sizes,path_to_csv,exp,step_percentage,eligibility=True):
    ...
    pattern_counter=0
    level=0
    plt.ylabel('Avg RelErr',size=20)
    plt.xlabel('% exhaustive runtime',size=20)
    map_furer_OBD_to_percentages_rel_errors={}
    map_furer_AD_to_percentages_rel_errors={}
    map_runtimes_seconds,map_final_embs=get_runtimes_embeddings_exhaustive(os.path.join(path_to_csv,'exhaustive_'+str(level)+".csv"),exp)
    if eligibility:
    ...
    plt.plot(x,average_relative_errors_furer_OBD,color='blue',linewidth=3.0)
    plt.plot(x,average_relative_errors_furer_AD,color='black',linewidth=3.0)
