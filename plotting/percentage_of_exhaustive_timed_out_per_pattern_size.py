...
def get_nr_timeouts(exhaustive_csv):
    counter=0
    counter_time_out=0
    nr_patterns=0
    with open(exhaustive_csv) as exh:
       ...
       for row in reader:
        ...
        if row["timeout"]=="True":
            counter_time_out+=1
    return counter_time_out,nr_patterns
...
def main(path_to_csvs,exp,levels):
    ...
    for l in levels:
        ...
        timeouts,patterns=get_nr_timeouts(exhaustive_csv)
        nr_timeout+=timeouts
    ...
    x=levels
    print "Overall number of timeouts: ",nr_timeout
    print "Overall number of timeouts (pers): ",(float(nr_timeout)/nr_patterns)*100
...
if __name__=='__main__':
  ...
  main(args.r,args.e,[4,5,6,7,8,9,10])