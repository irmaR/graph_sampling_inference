...
def main_multiple_pattern_size_plots(pattern_sizes,path_to_csv,exp,step_percentage,m,n,eligibility=True):
    ...
    for i in range(6):
        ...
        ax.plot(x,avg_ERR_AD_10,color='black',linewidth=3.0)
        ax.plot(x,avg_ERR_random,color='green',linewidth=3.0)
        print 'plotting: ',exp
        print avg_ERR_OBD
        print avg_ERR_AD
        ...
        if not exp=='dblp':
          ax.errorbar(x, avg_ERR_AD_10,yerr=stdev_furer_AD, fmt='o',color='black',linewidth=2.0)
        ax.errorbar(x, avg_ERR_random,yerr=stdev_random, fmt='o',color='green',linewidth=2.0)
        ax.set_ylim([0,1])
        ...
        if i==0:
            if not exp=='dblp':
               ax.legend(['FK-OBD', 'FK-AD','FK-AD-10','Random'],loc = 'upper right', bbox_to_anchor = (0,0.5,1.02,0.5),prop={'size':12}, ncol=2)
            else:
               ax.legend(['FK-OBD', 'FK-AD','Random'],loc = 'upper right', bbox_to_anchor = (0,0.5,1.02,0.5),prop={'size':12}, ncol=2)
