...
if __name__ == "__main__":
    ...
    parser.add_argument('-subset',default=False,action="store_true",help='dont use all the values of labels. Just top 10 from each node')
    ...
    parser.add_argument('-results_previous_level',help='path! patterns from previous level (containing all the batches)')
    parser.add_argument('-patterns_previous_level',help='path! patterns from previous level (containing all the batches)')
    ...
    if args.exp=='yeast':
        ...
        network=yeast.YEAST_GRAPH(data_graph,output_dir,args.nr_values_max,args.subset)
    if args.exp=='dblp':
    ...
    graphs_n_minus_1=directory_manager.load_selected_graphs(network,path_to_previous_level,args.results_previous_level)   
    print "Number of patterns from previous level: ",len(graphs_n_minus_1)
    ...
    patterns=generate_pattern.generate_patterns_of_depth_n_version_2(labels,graphs_n_minus_1,final_pattern_size,relation_predicate_names,output_dir,True,args.N)
    print "WRITING: ",len(patterns),"patterns"
    filtered_list,list_for_removal,pairs=gen_pat.find_isomorphic_graphs(patterns)
