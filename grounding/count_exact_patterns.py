...
import csv,math,sys,os,pickle,time
from graph_manipulator import visualization

def exact_counting(pattern,data_graph,OBdecomp,root_node,max_time):
    emb=exhaustive_approach_inf.get_nr_embeddings(data_graph, pattern, OBdecomp,root_node,max_time)
    return emb

def exact_counting_no_time_limit(pattern,data_graph,OBdecomp,root_node):
    emb=exhaustive_approach_inf.get_nr_embeddings_no_time_limit(data_graph, pattern, OBdecomp,root_node)
    return emb
...
def generate_csv_exact_counts(data_graph,target_graph,target_constant,target_attr,OBDTarget,root_node_target,patterns,OBDPatterns,indices,root_nodes_patterns,pattern_equivalences,non_equivalence,csvfile,fieldnames,time_dict_csv,max_time_exh):
    ...
    #count the patterns
    time_dict={}
    print "Running exhaustive per pattern for "+str(max_time_exh)+" seconds"
    counter=0
    ...
    #report times (in seconds) for grounding the patterns
