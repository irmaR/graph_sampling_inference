from graph_manipulator import graph_analyzer as ga
...
def find_all_groundings_of_predicates(data_graph,predicate1,predicate2):
    ...
    print "Values function: ",len(all_values1)
    all_nodes_constants=ga.get_all_possible_constants(D,predicate2)
    print "Nr constants: ",len(all_nodes_constants)
...
if __name__ == '__main__':
    ...
    for n in ground_pattern.nodes():
        print ground_pattern.node[n]
    print "------------------------"
    for n in pattern.nodes():
        print pattern.node[n]
