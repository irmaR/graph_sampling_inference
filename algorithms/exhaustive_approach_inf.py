...
def get_nr_embeddings(data_graph,pattern,OBdecomp,root_node_predicate_name,max_time):
    ...
    monitoring_marks = generate_monitoring_marks(max_time, max_time)
    lock = threading.Lock()
    p = Exhaustive_sampling_inf(data_graph, pattern, Plist, root_nodes, None, lock)
    m = None
    m = monitor_process.process_monitor.monitor_process_exhaustive(p, monitoring_marks, lock, None)
    mt = threading.Thread(target=m)
    mt.daemon = True
    mt.start()
    p.run()
    p.abort = True
    return experiments.globals.freq_dict_exhaustive

def get_nr_embeddings_no_time_limit(data_graph,pattern,OBdecomp,root_node_predicate_name):
    number_of_nodes_in_data = len(data_graph)
    root_node = None
    Plist = [item for sublist in OBdecomp for item in sublist]
    root_nodes = [x for x in data_graph.nodes() if data_graph.node[x]['predicate'] == root_node_predicate_name]
...
class Exhaustive_sampling_inf:
    ...
    def run(self):
        ...
        experiments.globals.freq_dict_exhaustive={}
