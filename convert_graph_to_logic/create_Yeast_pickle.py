...
def parse_db_yeast(file_name):
    ...
    #for n in D.nodes():
    #    if D.node[n]['predicate']=='interaction':
    #        print "NODE: ",D.node[n]
    #        print "Neighbours"
    #        for ne in D.neighbors(n):
    #            print D.node[ne]

    return D

if __name__ == '__main__':
    file_name_train = '/home/irma/work/DATA/DATA/yeast/Folds/fold1/train.db'
    file_name_test = '/home/irma/work/DATA/DATA/yeast/Folds/fold1/test.db'
    output_file_train = '/home/irma/work/DATA/DATA/yeast/Folds/fold1/train.gpickle'
    output_file_test = '/home/irma/work/DATA/DATA/yeast/Folds/fold1/test.gpickle'
    TrainGraph=parse_db_yeast(file_name_train)
    print "Nr nodes Train: ",len(TrainGraph.nodes())
    TestGraph = parse_db_yeast(file_name_train)


    #add equiv classes
    for n1 in TrainGraph.nodes():
        for n2 in TrainGraph.nodes():
            if TrainGraph.node[n1]['predicate'] == TrainGraph.node[n2]['predicate']:
                try:
                    if TrainGraph.node[n1]['value'] == TrainGraph.node[n2]['value']:
                        TrainGraph.add_edge(n1, n2)
                except KeyError:
                    continue
    for n1 in TestGraph.nodes():
        for n2 in TestGraph.nodes():
            if TestGraph.node[n1]['predicate'] == TestGraph.node[n2]['predicate']:
                try:
                    if TestGraph.node[n1]['value'] == TestGraph.node[n2]['value']:
                        TestGraph.add_edge(n1, n2)
                except KeyError:
                    continue
