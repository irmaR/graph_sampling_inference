...
def get_data_graph_summary(graph):
    ...
    for k in neighbours.keys():
        ...
        #if 'value' in graph.node[node].keys():
        #        print "HAS VALUE: ",graph.node[node]
        #if not (graph.node[node]['predicate'],graph.node[node]['label']) in info_tuples: 
        #    info_tuples.append((graph.node[node]['predicate'],graph.node[node]['label']))
    #for t in info_tuples:
    #    print t
    #for p in has_values_in_patterns:
    #  print p
